#pyinstaller .venv\_dod -decimal.py --onefile
import os
import openpyxl
...
def create_table(document, rows, table_title):
    ...
    def add_table_row(table, data):
        ...
        for row_data in data:
            ...
            for i, value in enumerate(row_data):
                cells[i].text = data[i] #if data[i] is not None else ''
                set_font(cells[i].paragraphs[0])
    ...
    table = document.add_table(rows=1, cols=6) #NIFA has extra columns
    table.autofit = True
    ...
    #Set the headers in the top row
    nifa_headers = ["Name", "Agency", "Amount", "Dates", "Time", "Title"]
    for i, header in enumerate(nifa_headers):
    ...
    for row in rows:
        data = [row[39],row[40], row[18],row[40],row[17],row[1]]
        add_table_row(table, data)
...
def create_document():
    ...
    run = other_support_text.add_run("NIFA Current and Pending")
    run.bold = True
    run.font.size = Pt(14)
...
try:
    category_column = headers['NIFA/AFRI/USDA Category'] #set the department's category
except KeyError:
...
current_projects = fill_projects(sheet, 41, category_column, 'Active')
...
if current_projects == []:
    ...
    create_table(document, current_projects, 'Active Projects')
