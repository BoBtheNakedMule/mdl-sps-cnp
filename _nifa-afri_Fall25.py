...
# Main code
version = "SPS-NIFA/AFRI-20251002"
print(20 * "*")
...
def create_table(doc, projects, title, category):
    ...
    headers = ['Name (List/PD #1 first)', 'SUPPORTING AGENCY AND AGENCY ACTIVE AWARD/PENDING PROPOSAL NUMBER', 'TOTAL $ AMOUNT', 'EFFECTIVE AND EXPIRATION DATES', ' % OF TIME COMMITTED', 'TITLE OF PROJECT']
    ...
    # Set category row in the second column while blanking out the first one and the last four.
    category_cells = table.rows[1].cells
    category_cells[0].text = ""
    ...
    for index, (person, agency, amount, dates, time, title) in enumerate(projects):
        ...
        row_cells[5].text = str(other_column_blank(f"{title}\n", "Title"))
...
def create_document():
    ...
    add_formatted_paragraph(sec_cell, "Each project director/principal investigator (PD/PI) and other senior personnel specified in the Request for Applications (RFA). For Agriculture and Food Research Initiative (AFRI) applications, completion of this is only required for PDs/PIs and CoPDs/CoPIs.")
